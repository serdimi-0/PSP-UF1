...
int condicionarClau(char* k) {
	unsigned char aux[8];
	unsigned long long int p, d1, d2, d;

	for (int i = 0; i < 8; i++) aux[i] = key[i % strlen(k)];				// Còpiem la clau repetidament fins a 8 bytes
	memcpy(&p, aux, 8);
	d1 = d2 = p;
	d1 &= 0xaaaaaaaaaaaaaaaa;
	d2 &= 0x5555555555555555;
...
unsigned char* Des::Encrypt(unsigned char* Text1, int len, char* Id)
{
	int i, a1, j, nB, m, iB, k, K, B[8], n, t, d, round;
	unsigned char* Text = new unsigned char[1000];

	if (Id) condicionarClau(Id);
	...
	Text[i] = '\0';
	keygen();
	for (iB = 0, nB = 0, m = 0; m < (len / 8); m++) //Repeat for TextLenth/8 times.  
	{
...
unsigned char* Des::Decrypt(unsigned char* Text1, int len)
{
	int i, a1, j, nB, m, iB, k, K, B[8], n, t, d, round;
	unsigned char* Text = new unsigned char[1000];
	unsigned char ch;
	...
	for (iB = 0, nB = 0, m = 0; m < (len / 8); m++) //Repeat for TextLenth/8 times.  
	{
		...
		for (i = 0; i < 8; i++)
		{
			...
			final[mc++] = (unsigned char)d;
			k = 128;
			d = 0;
		}
	} //for loop ends here  
	final[mc] = '\0';
	char* final1 = (char*)malloc(len + 1);
...
int main()
{
	...
	unsigned char* enc;
	unsigned char* dec;
	unsigned char text[16] = "Hello World\0";
	enc = d.Encrypt(text, 16);
	dec = d.Decrypt(enc, 16);
